...
[![星标](https://img.shields.io/github/stars/Yersmagit/cw-lessons-displayer?style=for-the-badge&color=orange&label=星标)](https://github.com/Yersmagit/cw-lessons-displayer)
...
[![下载量](https://img.shields.io/github/downloads/Yersmagit/cw-lessons-displayer/total.svg?label=下载量&color=green&style=for-the-badge)](https://github.com/Yersmagit/cw-lessons-displayer)
...
- **在快要放学的时候显示明天的课程**
  ...
  - **在 `./config/` 目录下打开 `data.json` ，在其中找到键 `"tomorrow_course"`，可以编辑显示明天课程的相关设置。**
    ...
    ```json
    {
        ...
        "switch": "False",
        "time_remaining": "60",
        "start_time_limit": "18:00"
      }
    }
    ```
  - `"switch"` 是此功能的开关。只有当它的值为 `"True"` 时，才会开启显示明日课程的功能。
...
- **自动化打开或关闭熄屏模式或白板模式**
  - **在 `./config/` 目录下打开 `data.json` ，在其中找到键 `"events"`，可以编辑在其中相应课程的自动化设置。**
